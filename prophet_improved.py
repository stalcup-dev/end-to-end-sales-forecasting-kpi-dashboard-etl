...
for idx, sku in enumerate(eligible_skus, 1):
    ...
    print(f"      → MAE: {mae_test:.1f} | MAPE: {mape_test:.1f}% | RMSE: {rmse_test:.1f} | Bias: {bias_test:.1f}")
...
if len(metrics_df) > 0:
    ...
    for _, row in metrics_df.iterrows():
        print(f"{row['sku']:<25} {row['test_mape_pct']:>8.1f}% {row['test_mae']:>9.1f} {row['test_rmse']:>9.1f} {row['test_bias']:>9.1f} {row['test_coverage_pct']:>10.1f}%")
    
    print("-" * 90)
    print(f"{'MEDIAN':<25} {metrics_df['test_mape_pct'].median():>8.1f}% {metrics_df['test_mae'].median():>9.1f} {metrics_df['test_rmse'].median():>9.1f} {metrics_df['test_bias'].median():>9.1f} {metrics_df['test_coverage_pct'].median():>10.1f}%")
