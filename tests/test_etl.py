...
class TestDataCleaning:
    ...
    def test_date_parsing(self):
        ...
        df = pd.DataFrame(
            {"date": ["2024-01-01", "2024-01-02", "2024-01-03"], "value": [100, 200, 300]}
        )
    ...
    def test_negative_values_removed(self):
        ...
        df = pd.DataFrame({"units_sold": [10, -5, 20, -1, 30], "sku": ["A", "A", "B", "B", "C"]})
    ...
    def test_null_handling(self):
        ...
        df = pd.DataFrame(
            {"units_sold": [10, None, 20, np.nan, 30], "order_value": [100, 200, None, 400, 500]}
        )
    ...
    def test_outlier_clipping(self):
        ...
        df = pd.DataFrame(
            {"sku": ["A"] * 100, "y": list(range(1, 100)) + [1000]}  # 1000 is outlier
        )
...
class TestDataValidation:
    ...
    def test_sku_eligibility_filter(self):
        ...
        sku_stats = (
            df.groupby("sku")
            .agg(first_date=("ds", "min"), last_date=("ds", "max"), total_units=("y", "sum"))
            .reset_index()
        )
        sku_stats["span_days"] = (sku_stats["last_date"] - sku_stats["first_date"]).dt.days

        # Filter eligible
        eligible = sku_stats[(sku_stats["span_days"] >= 730) & (sku_stats["total_units"] > 500)]
...
class TestForecastingPrep:
    ...
    def test_prophet_column_names(self):
        ...
        df = pd.DataFrame({"date": ["2024-01-01", "2024-01-02"], "total_units_sold": [100, 200]})
