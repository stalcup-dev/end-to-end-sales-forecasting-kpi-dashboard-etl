...
if __name__ == "__main__":
    ...
    run("dbt deps", cwd=ROOT / "dbt")
    run("dbt run",  cwd=ROOT / "dbt")

    # 2) Export mart_sales_summary to CSV (for Power BI or archive)
    run(f'{PY} scripts/refresh_actuals.py', cwd=ROOT)

    # 3) Build Prophet forecasts & write table public.simple_prophet_forecast
    run(f'{PY} scripts/prophet_forecast.py', cwd=ROOT)

    # 4) Optional: quick data quality snapshot
    run(f'{PY} scripts/checkcsv.py', cwd=ROOT)
